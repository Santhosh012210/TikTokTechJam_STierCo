...
Usage:
  ...
  python -m mle_agent.harness.main --max-iter 10 --wall-hours 0.5 --builder-turns 2

  # Production run (after the agent harness is verified)
  python -m mle_agent.harness.main --max-iter 50 --wall-hours 4 --builder-turns 10
"""
import argparse
import difflib
import json
...
from mle_agent.harness.provider import validate_provider_environment
from mle_agent.harness.validator import scan_candidate_source
from mle_agent.research_agent.builder import BuilderResult, run_builder_session
from mle_agent.research_agent.knowledge import list_methods
from mle_agent.research_agent.search.tree import SearchTree
from mle_agent.research_agent.strategist import run_strategist_session
...
def assemble_strategist_log(iteration: int, strat, config: Config) -> dict:
    from mle_agent.research_agent.strategist import StrategistResult
    return {
        "iteration":          iteration,
...
"""
    ...
    print("\nNext steps:")
    print(f"  1. Validate the log: python -m mle_agent.harness.validator {logger.path}")
