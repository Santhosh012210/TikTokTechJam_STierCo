"""
Official KuaiRand-Pure evaluation script. All evaluation conventions are fixed here; do not modify them.
...
Ranking scope : rank only each user's impressions in the evaluation set; no full-corpus retrieval
Zero-positive
users         : nDCG is 0.0 and remains in the mean (consistent with CWM)
                GAUC includes only users with 0 < positives < impressions, weighted by positives
...
def ndcg_at_k(labels, k):
    """Labels must already be sorted by predicted score in descending order."""
...
def evaluate(user_ids, labels, scores, k=5):
    """Return {'GAUC': ..., 'nDCG@5': ..., 'primary': ...}; primary is their ranking mean."""
