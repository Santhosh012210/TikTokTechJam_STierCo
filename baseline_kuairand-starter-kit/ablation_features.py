"""Add CWM's 13 feature fields to test whether user-side features help the FM."""
...
def build(mode):
    """mode: 'base'=current 5 fields / 'item'=add item-side only / 'cwm13'=all 13 CWM fields"""
...
for mode,desc in [('base','5 fields (current kit)'),('item','+4 item-side = 9 fields'),('cwm13','all 13 CWM fields')]:
    ...
    print(f"{desc:28s} ({nf:2d} fields) | test GAUC {g:.4f} | nDCG@5 {n5:.4f} | primary {pr:.4f} ± {sd:.4f}")
